...
export function constructNFTCollectionCode(
    ...
    const collectionConstantName = collectionIdentifierParts[2].toLowerCase() + collectionIdentifierParts[3];
    ...
    const transformedInterfaces = fullyQualifiedInterfaces.map(item => item.replace(/A\.[^.]+./g, ""));
