...
transaction(
    ...
    let minter: &Apples.NFTMinter
    let recipientCollectionRef: &{NonFungibleToken.CollectionPublic}

    let mintingIDBefore1: UInt64

    prepare(signer: AuthAccount) {
        self.mintingIDBefore1 = Apples.totalSupply

        self.minter = signer.borrow<&Apples.NFTMinter>(from: Apples.MinterStoragePath)
            ?? panic("Account does not store an object at the specified path")

        // Borrow the recipient's public NFT collection reference
        self.recipientCollectionRef = getAccount(recipient)
            .getCapability(Apples.CollectionPublicPath)
    ...
    post {
        ...
        Apples.totalSupply == self.mintingIDBefore1 + 1: "The total supply should have been increased by 1"
    }
}
