...
transaction(
    ...
    let minter: &Black.NFTMinter
    let recipientCollectionRef: &{NonFungibleToken.CollectionPublic}

    let mintingIDBefore1: UInt64

    prepare(signer: AuthAccount) {
        self.mintingIDBefore1 = Black.totalSupply

        self.minter = signer.borrow<&Black.NFTMinter>(from: Black.MinterStoragePath)
            ?? panic("Account does not store an object at the specified path")

        // Borrow the recipient's public NFT collection reference
        self.recipientCollectionRef = getAccount(recipient)
            .getCapability(Black.CollectionPublicPath)
    ...
    post {
        ...
        Black.totalSupply == self.mintingIDBefore1 + 1: "The total supply should have been increased by 1"
    }
}
