...
transaction(
    ...
    let minter: &Oranges.NFTMinter
    let recipientCollectionRef: &{NonFungibleToken.CollectionPublic}

    let mintingIDBefore1: UInt64

    prepare(signer: AuthAccount) {
        self.mintingIDBefore1 = Oranges.totalSupply

        self.minter = signer.borrow<&Oranges.NFTMinter>(from: Oranges.MinterStoragePath)
            ?? panic("Account does not store an object at the specified path")

        // Borrow the recipient's public NFT collection reference
        self.recipientCollectionRef = getAccount(recipient)
            .getCapability(Oranges.CollectionPublicPath)
    ...
    post {
        ...
        Oranges.totalSupply == self.mintingIDBefore1 + 1: "The total supply should have been increased by 1"
    }
}
